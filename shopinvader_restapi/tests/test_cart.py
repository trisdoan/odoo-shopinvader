...
class CartCase(CommonCase):
    ...
    def setUp(self):
        ...
        self.fposition = self.env.ref("shopinvader_restapi.fiscal_position_2")
        self.default_fposition = self.env.ref("shopinvader_restapi.fiscal_position_0")
...
class AnonymousCartCase(CartCase, CartClearTest):
    def setUp(self, *args, **kwargs):
        ...
        self.cart = self.env.ref("shopinvader_restapi.sale_order_1")
    ...
    def test_anonymous_cart_then_sign(self):
        cart = self.cart
        invader_partner = self.env.ref("shopinvader_restapi.shopinvader_partner_1")
...
class ConnectedCartCase(CommonConnectedCartCase, CartClearTest):
    ...
    def test_set_shipping_address_no_default_invocing(self):
        ...
        invoice_addr = self.env.ref("shopinvader_restapi.partner_1_address_2")
...
class ConnectedCartNoTaxCase(CartCase):
    def setUp(self, *args, **kwargs):
        ...
        self.cart = self.env.ref("shopinvader_restapi.sale_order_3")
        self.shopinvader_session = {"cart_id": self.cart.id}
        self.partner = self.env.ref("shopinvader_restapi.partner_2")
        self.address = self.env.ref("shopinvader_restapi.partner_2_address_1")
        with self.work_on_services(
            partner=self.partner, shopinvader_session=self.shopinvader_session
        ) as work:
            self.service = work.component(usage="cart")
